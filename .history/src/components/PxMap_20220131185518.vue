...
import { usePlaces, useMaps, useDefaultPlaces } from "@/composables";
...
export default {
  ...
  setup() {
    ...
    const { coordinates, location } = useDefaultPlaces();
    ...
    const initMap = async () => {
      ...
      const myLocationMarker = new mapboxgl.Marker()
        ...
        .addTo(map);
      const popup1 = new mapboxgl.Popup().setLngLat(
        coordinates[0].setHTML(
          `<h3>${location[0]}</h3><p>Este es un popup de muestra</p>`
        )
      );
      const marker1 = new mapboxgl.Marker()
        ...
        .addTo(map);
      setMap(map);
      const popup2 = new mapboxgl.Popup().setLngLat(
        coordinates[1].setHTML(
          `<h3>${location[1]}</h3><p>Este es un popup de muestra</p>`
        )
      );
